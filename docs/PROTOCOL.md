...
| 0x06 | OPEN_OK   | server -> client| `init_window(4)` BE - стартовое окно отдачи (см. Flow control) |
| 0x07 | OPEN_ERR  | server -> client| utf8-причина                     |
| 0x08 | WINDOW_UPDATE | server -> client| `credit(4)` BE - +N байт к окну client->upstream |
...
## Flow control (client -> upstream)

Направление аплоада (client -> server -> upstream) имеет per-stream оконный
контроль, чтобы прокси НИКОГДА не резал передачу и при этом не держал в
памяти весь файл. Без него большой аплоад на upstream, который сливается
медленнее туннеля, переполнял буфер: старый сервер дропал такой стрим, и
аплоад обрывался (получатель видел 0 байт / нет MIME).

Механика (как в HTTP/2 / yamux):

- В `OPEN_OK` сервер отдаёт стартовое окно `init_window` (4 байта BE,
  сейчас 4 MiB). Клиент держит в полёте не больше окна непокрытых кредитом
  байт.
- По мере слива в upstream сервер шлёт `WINDOW_UPDATE` с кредитом (число
  слитых байт; батчами ~по половине окна, плюс обязательный flush когда
  очередь опустела - чтобы хвост аплоада не завис). Клиент прибавляет
  кредит к окну.
- Когда окно исчерпано, запись клиента блокируется - backpressure доходит
  до upload-потока SDK, и он читает файл со скоростью upstream'а. Память на
  сервере ограничена ~окном на стрим (несколько МБ) независимо от размера
  файла: аплоад на 100-500 МБ проходит потоково и без потерь.
- Read-loop сервера НИКОГДА не блокируется на медленном стриме (нет HoL на
  остальные стримы сессии) и НИКОГДА не дропает идущий аплоад. Жёсткие
  per-session / global байтовые капы остаются только как защита памяти от
  клиента, который окно игнорирует (старая сборка) или умышленно флудит -
  flow-controlled клиент до них не доходит.

Обратная совместимость: старый клиент игнорирует `WINDOW_UPDATE` (unknown
type -> skip) и payload `OPEN_OK`, работает как раньше (через байтовый кап);
новый клиент против старого сервера получает `OPEN_OK` без окна и остаётся в
прежнем unbounded-режиме. Менять порядок выката не обязательно, но сервер
логично катить первым.

Маскировка: `WINDOW_UPDATE` идёт через ту же AEAD + PADME-лесенку, что и
любой фрейм - на wire это 256-байтная рунга, неотличимая от PONG / мелкого
DATA, а байт типа зашифрован. Каденция кредитов дата-зависимая (по мере
слива), не фиксированный тик, и это не тугой ответ «на фрейм за 1мс» (jitter
не нужен). Большой аплоад с редкими мелкими реверс-фреймами выглядит как
обычный HTTP/2 / QUIC аплоад с их WINDOW_UPDATE / ACK.
...
- Не имеет congestion-control сверх TCP на уровне сессии (один сокет = одно
  TCP-окно). Но направление аплоада (client -> upstream) имеет per-stream
  оконный flow control (см. «Flow control»), поэтому большой аплоад больше
  не переполняет буфер и не вызывает HoL на остальные стримы. Направление
  скачивания (upstream -> client) пока без оконного контроля: медленный
  читающий клиент может придержать общий writeMu и подтормозить встречные
  стримы (данные не теряются, только латентность).
