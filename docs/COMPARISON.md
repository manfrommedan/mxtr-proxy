...
## Коротко: чем mxtr реально лучше

То, что в апреле 2026 убило MTProxy, на mxtr не работает:

- **Нет детерминированного отпечатка - нет массового бана.** MTProxy
  завалили одним правилом по JA3 fake-TLS (`0xfe02` + кривой key_share):
  одна сигнатура - забанен весь класс разом. mxtr ходит по настоящему
  TLS-1.3, его handshake - чистая энтропия без магических байт. Единого
  правила, ловящего «все mxtr», написать не из чего; ТСПУ остаётся
  дорогое пер-IP зондирование, а не дешёвый масс-фильтр на всю страну.
- **Не выдаёт свой стек.** В ALPN только `http/1.1` (h2 убран, чтобы
  Go-http2 SETTINGS не палил сервер как Go), ответы камуфляжа собраны
  вручную, не через Go net/http. На активный зонд - поведение обычного
  http/1.1-сервера: 403/404/500 одного из 6 семейств, `/robots.txt` 200,
  мусор - `400 Bad Request`. Молчаливого hang-тели нет.
- **Каждый деплой уникален.** Heartbeat cadence выводится из PSK,
  CN-сертификата и cloak-family случайны и персистентны. Отпечаток
  одного бокса на другие не переносится.
- **Нечего блокировать оптом.** Ни публичных списков, ни центрального
  сервера - каждый поднимает свой VPS и раздаёт ссылку своему кругу.
- **Утечка PSK не раскрывает прошлый трафик.** Три независимых слоя
  forward secrecy: внешний TLS-1.3 ECDHE, внутренний TLS до matrix.org,
  Matrix E2EE поверх.

Честная граница: невидимым это mxtr не делает. Остаются дорогие пути -
точечное зондирование (самый дешёвый остаточный тель это self-signed
серт, закрывается своим доменом + LE) и блокировка по IP. Но **массовая
MTProto-style охота по одной сигнатуре на mxtr не масштабируется** - а
это и есть то, чем сегодня жгут обходы.
